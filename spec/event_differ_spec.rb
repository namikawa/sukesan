...
RSpec.describe EventDiffer do
  def event(source:, title:, start:, cancelled: false)
    Event.new(
      source: source,
      title: title,
  ...
  describe ".outlook_only" do
    ...
    it "キャンセル済み（isCancelled）の Outlook イベントは同期候補から除外する" do
      outlook = [
        event(source: "outlook", title: "打合せ", start: "2026-06-20T10:00:00+09:00", cancelled: true),
        event(source: "outlook", title: "商談", start: "2026-06-21T10:00:00+09:00")
      ]

      result = described_class.outlook_only(google_events: [], outlook_events: outlook)
      expect(result.map(&:title)).to eq(["商談"])
    end

    it "Google 側の「Fw:」を無視して同一イベントとみなし除外する" do
      google = [event(source: "google", title: "Fw: 定例MTG", start: "2026-06-20T10:00:00+09:00")]
      outlook = [event(source: "outlook", title: "定例MTG", start: "2026-06-20T10:00:00+09:00")]
